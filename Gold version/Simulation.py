...
if choice == "1":
    ...
    while f.is_int(sigma_dens) == False or float(sigma_dens) < 0 : #Verify if the sigma parameter is a positive float
        sigma_dens = input("Which value do you want to give to the fractal parameter (> 0.1)?\n")
    ...
    for i in range(number_of_grain): #Loop that create and plot histogram for each grain
        ...
        results = np.loadtxt("Results_Files/results_Grain_N100_S{}p{}_B3p0_{}.txt".format(S, p, i)) #Loading the file who contains the energy value
        plt.subplot(1,number_of_grain,i+1) #Craeting subplot in order to have hist side by side
        plt.hist(results, bins = 30, range = (0,13), edgecolor = "black", color = "red", label = "grain n°{}".format(i)) #The range corresponding to the range of photons energies
        plt.legend(loc="upper left", bbox_to_anchor=[0, 1], fancybox=True)
    plt.suptitle("Different grain with the same fractale parameters (1000000 photons were sent on each grain)")
    plt.show()
    ...
    while f.is_int(fractale_transit) == False or float(fractale_transit) < 0 :
    ...
    for i in range(number_of_grain): #Hist are ploted one over the other, thanks to the sorting
        results = np.loadtxt("Results_Files/results_Grain_N100_S{}p{}_B3p0.txt".format(S[i], p[i]))
        plt.hist(results, bins = 30, range = (0,13), edgecolor = "black", label = "sigma = {}".format(sigma_dens[i]))
        plt.xlabel("Energy (eV)")
        plt.ylabel("Number of photo_electron who escaped from the grain")
    plt.title("A fractal and a sperical grain (1000000 photons were sent on each grain)")
    plt.legend()
    plt.show()
    ...
    for i in range(number_of_grain):
        transit = input("Which value do you want to assign to the fractal parameter n°{}?\n".format(i+1))
    ...
    for i in range(number_of_grain):
        ...
        plt.xlabel("Energy (eV)")
        plt.ylabel("Number of photo_electron who escaped from the grain")
    plt.title("Different grain with different fractal properties (1000000 photons were sent on each grain)")
    plt.legend()
    plt.show()
    ...
    for i in range(number_of_grain):
        transit = input("Which value do you want to assign to the grain radius n°{}? (between 1e-7 and 1e-8 for acceptable results)\n".format(i+1))
        while f.is_int(transit) == False or float(transit) < 0 :
            transit = input("Which value do you want to assign to the grain radius n°{}? (between 1e-7 and 1e-8 for acceptable results)\n".format(i+1))
    ...
    for i in range(number_of_grain):
        results = np.loadtxt("Results_Files/results_Grain_N100_S{}p{}_B3p0_{}.txt".format(S, p, i))
        plt.hist(results, bins = 30, range = (0,13), edgecolor = "black", label = "Radius = {}".format(GRAIN_RADIUS[i]))
        plt.xlabel("Energy (eV)")
        plt.ylabel("Number of photo_electron who escaped from the grain")
    plt.suptitle("Different grain size with the same fractale parameters (1000000 photons were sent on each grain)")
    plt.legend()
    plt.show()
